...
struct CAMERA {
	...
	void Draw() {
		glm::vec3 cameraPos = { camerapos[3].x,camerapos[3].y,camerapos[3].z }; //--- 카메라 위치
...
GLvoid Timer(int value)
{
	...
	case 5:
		if (button_y)
		{
			...
			//dice[0].move_pos.z = camerapos[3].z ;
			//dice[1].move_pos.z = camerapos[3].z ;
			//dice[0].revolutionAngle[1] += 2.0f;
			//dice[1].revolutionAngle[1] += 2.0f;
...
GLvoid KeyBoardFunc(unsigned char key, int x, int y)
{
	...
	case 'T':
		...
		break;
	case 'z':
		camera.camerapos[3].z += 0.1f;
		break;
	case 'Z':
		camera.camerapos[3].z -= 0.1f;
		break;
	case 'x':
		camera.camerapos[3].x += 0.1f;
		break;
	case 'X':
		camera.camerapos[3].x -= 0.1f;
		break;
	...
	case 'R':
		...
		break;
	case 's':
		//camerapos[3].x = -0.5f; camerapos[3].y = 1.0f; camerapos[3].z = 3.0f;
		//CRe_matrix = glm::mat4(1.0f); //단위행렬로 초기화
	...
	case 'c':
		...
		camera.CR_matrix = glm::mat4(1.0f);
		Map[1].move_pos.x = 0;
		break;
	case 'q':
		glutLeaveMainLoop();
