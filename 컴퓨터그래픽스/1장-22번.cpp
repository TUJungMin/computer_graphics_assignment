...
vec3 door_vertex[8] = { { -0.5,-0.5,0.5 }, {  0,-0.5,0.5 },{0,0.5,0.5 }, { -0.5,0.5,0.5}, { 0,-0.5,0.5 }, {  0.5,-0.5,0.5 },{0.5,0.5,0.5 }, { 0,0.5,0.5} };
...
struct DOOR {
	...
	vec3 start_vertex[8];
	void InitVbo() {
		std::copy(door_vertex, door_vertex + 8, start_vertex);
		glGenBuffers(1, &g_vbo); // VBO 생성
		glBindBuffer(GL_ARRAY_BUFFER, g_vbo); // VBO를 바인딩

		glBufferData(GL_ARRAY_BUFFER, sizeof(door_vertex), door_vertex, GL_STATIC_DRAW);
	...
	void open() 
	{
		if (open_button) {
			if (angle[0] < 180) {
				angle[0]++;
				door_vertex[1].x = start_vertex[0].x + 0.5f * glm::cos(glm::radians(angle[0]));
				door_vertex[1].z = start_vertex[0].z + 0.5f * glm::sin(glm::radians(angle[0]));
				door_vertex[2].x = start_vertex[3].x + 0.5f * glm::cos(glm::radians(angle[0]));
				door_vertex[2].z = start_vertex[3].z + 0.5f * glm::sin(glm::radians(angle[0]));
			}
			if (angle[1] < 180) {
				angle[1]++;
				door_vertex[4].x = start_vertex[5].x - 0.5f * glm::cos(glm::radians(angle[0]));
				door_vertex[4].z = start_vertex[5].z + 0.5f * glm::sin(glm::radians(angle[0]));
				door_vertex[7].x = start_vertex[6].x - 0.5f * glm::cos(glm::radians(angle[0]));
				door_vertex[7].z = start_vertex[6].z + 0.5f * glm::sin(glm::radians(angle[0]));
			}
		}
		else
		{
...
GLvoid KeyBoardFunc(unsigned char key, int x, int y)
{
	...
	case 'o':
		...
		door.open_button = !door.open_button;
		break;
	case 'z':
		camera.pos.z += 0.1f;
		break;
