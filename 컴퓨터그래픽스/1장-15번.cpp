...
GLvoid KeyBoardFunc(unsigned char key, int x, int y)
{
	...
	case 's':
		if (dice[0].Dodraw) {
			...
			dice[0].R_matrix = glm::rotate(dice[0].R_matrix, glm::radians(-30.0f), glm::vec3(0, 1, 0));//y축 -30도
			dice[0].R_matrix = glm::rotate(dice[0].R_matrix, glm::radians(30.0f), glm::vec3(1, 0, 0));//x축 -30도
			...
			pyramid[0].R_matrix = glm::rotate(pyramid[0].R_matrix, glm::radians(-30.0f), glm::vec3(0, 1, 0));//y축 -30도
			pyramid[0].R_matrix = glm::rotate(pyramid[0].R_matrix, glm::radians(30.0f), glm::vec3(1, 0, 0));//x축 -30도
...
GLvoid Timer(int value)
{
	if (dice[0].Dodraw) {
		if (dice[0].button_x)
		{
			dice[0].R_matrix = glm::rotate(dice[0].R_matrix, glm::radians(dice[0].rotate_x), glm::vec3(1, 0, 0));//x축 -30도
		}
		...
		{
			pyramid[0].R_matrix = glm::rotate(pyramid[0].R_matrix, glm::radians(pyramid[0].rotate_x), glm::vec3(1, 0, 0));//x축 -30도
		}
